...
def safe_find_first(elem, paths, ns):
    """Intenta encontrar el primer elemento que coincida con alguna de las rutas."""
    for p in paths:
...
def apply_metadata_estado(df: pd.DataFrame, meta_df: pd.DataFrame) -> pd.DataFrame:
    """Cruza el df con la metadata para llenar la columna 'Estado' (Vigente/Cancelado)."""
    ...
    if 'UUID' in df.columns:
        ...
        if 'Estado_meta' in merged.columns:
            if 'Estado' in df2.columns:
                merged['Estado'] = merged['Estado_meta'].where(
                    (merged['Estado_meta'].notna()) & (merged['Estado_meta'] != ''),
                    merged['Estado']
                )
...
def parse_xml_invoice_onerow(xml_text):
...
def process_invoice_files(uploaded_files):
    ...
    for idx, uploaded_file in enumerate(uploaded_files):
        try:
            ...
            invoice = parse_xml_invoice_onerow(xml_content)
            if invoice:
    ...
    if all_invoices:
        ...
        df['Fecha'] = df['Fecha'].dt.strftime('%Y-%m-%d %H:%S')
        return df, errors
    return None, errors
...
def process_payment_files(uploaded_files):
    ...
    for idx, uploaded_file in enumerate(uploaded_files):
        try:
            ...
            if payments:
                ...
                status_text.text(f"Procesado: {uploaded_file.name} ({len(payments)} pagos)")
            else:
    ...
    if all_payments:
        ...
        columnas_ordenadas = ['Receptor', 'Fecha', 'Mes', 'RFC Receptor', 'Folio Pago', 
                             'Folio Documento', 'UUID Factura', 'Monto Pagado', 'Estado', 'UUID']
...
tab1, tab2, tab3 = st.tabs(["📄 Facturas", "💰 Pagos", "📊 Análisis"])
...
with tab1:
    ...
    if meta_file is not None:
        try:
            ...
            st.markdown(f'<div class="status-success">✅ Metadata cargada: {len(meta_df)} UUIDs</div>', unsafe_allow_html=True)
    ...
    if uploaded_files_inv:
        st.markdown(f'<div class="status-info">📁 {len(uploaded_files_inv)} archivos seleccionados</div>', unsafe_allow_html=True)
                ...
                if df is not None and len(df) > 0:
                    ...
                    with pd.ExcelWriter(output, engine='openpyxl') as writer:
                        ...
                        column_widths = {
                            ...
                            'Emisor': 35, 'Descripcion': 60, 'Cantidad': 12, 'Importe': 12,
                            'IVA': 12, 'ISR Retenido': 15, 'IVA Retenido': 15, 'IEPS': 12,
                            'Subtotal': 12, 'Total': 12, 'Moneda': 10, 'Estado': 14
                        }
                    ...
                    output.seek(0)
                    st.markdown(f'<div class="status-success">✅ {len(df)} facturas procesadas y ordenadas cronológicamente</div>', unsafe_allow_html=True)
                    st.download_button(
                    ...
                    if errors:
                        st.markdown(f'<div class="status-warning">⚠️ Advertencias: {len(errors)} archivos con problemas</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    if meta_file_pay is not None:
        try:
            ...
            st.markdown(f'<div class="status-success">✅ Metadata cargada: {len(meta_df_pay)} UUIDs</div>', unsafe_allow_html=True)
            with st.expander("Vista previa metadata"):
